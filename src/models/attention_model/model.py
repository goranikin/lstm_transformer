...
class AttentionModel(nn.Module):
    ...
    def solve_mis(
        ...
        node_glimpse_key, node_glimpse_value, node_logit_key = self.W_node_proj(
            h
        ).chunk(
            3,
            dim=-1,
        ...
        for _ in range(num_nodes + 1):
            ...
            context = torch.cat([h_bar, prev_emb, mis_context], dim=-1)
    ...
    def supervised_loss(
        ...
        problem = self._resolve_problem(batch, problem)
        if problem == "tsp":
            return self.supervised_tsp_loss(batch, label_smoothing=label_smoothing)
        if problem == "mis":
            return self.supervised_mis_loss(batch, label_smoothing=label_smoothing)
        raise ValueError(f"Unsupported problem: {problem}")

    def supervised_tsp_loss(
        self,
        batch: dict[str, torch.Tensor],
        *,
        label_smoothing: float = 0.0,
        ...
        if label_smoothing:
            raise ValueError("TSP label smoothing is not supported by AttentionModel")
        loc = self._require_tensor(batch, self.loc_key)
    ...
    def supervised_mis_loss(
        ...
        *,
        ...
        if label_smoothing:
            target_set = target_set * (1.0 - label_smoothing) + 0.5 * label_smoothing
    ...
    def mis_node_logits(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        h, _ = self.encoder(
            self.mis_node_embed(self._mis_node_features(adjacency))
            + self.mis_node_bias,
        )
