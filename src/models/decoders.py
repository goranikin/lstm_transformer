...
from src.core import EncoderOutput, ProblemDecodeState, SolutionOutput
...
class AutoregressiveDecoder(nn.Module):
    ...
    def _action_embeddings(
        ...
        return torch.where(
            (action >= 0).unsqueeze(1), gathered, placeholder.expand(batch_size, -1)
        )
    ...
    def _append_stop_key(
        ...
        return torch.cat(
            [keys, stop_key.view(1, 1, -1).expand(keys.size(0), 1, -1)], dim=1
        )
