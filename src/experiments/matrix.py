...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Expand the src architecture/problem/mode/seed matrix."
    )
    ...
    parser.add_argument("--output-root", default="outputs/src/matrix")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    seeds = tuple(
        int(seed) for seed in split_csv(args.seeds, tuple(map(str, DEFAULT_SEEDS)))
    )
