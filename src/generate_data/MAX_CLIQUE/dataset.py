...
class MaxCliqueDataset(Dataset):
    def __init__(
        self,
        path: str,
        ...
        for index, record in enumerate(self.records):
            if record.get("problem") != "maximum_clique":
                raise ValueError(f"Record {index} is not a Maximum Clique instance")
            if (
...
def collate_max_clique(items: list[dict[str, Any]]) -> dict[str, Any]:
    if not items:
        raise ValueError("Cannot collate an empty Maximum Clique batch")
    keys = items[0].keys()
    batch: dict[str, Any] = {}
    for key in keys:
