...
class VertexCoverDataset(Dataset):
    def __init__(
        ...
        for index, record in enumerate(self.records):
            if record.get("problem") != "minimum_vertex_cover":
                raise ValueError(
                    f"Record {index} is not a Minimum Vertex Cover instance"
                )
            if (
...
def collate_vertex_cover(items: list[dict[str, Any]]) -> dict[str, Any]:
    if not items:
        raise ValueError("Cannot collate an empty Minimum Vertex Cover batch")
    keys = items[0].keys()
    batch: dict[str, Any] = {}
    for key in keys:
