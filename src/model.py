...
class NCOModel(nn.Module):
    ...
    def encode(self, batch: dict[str, Any]):
        ...
        return self.encoder(
            node_features, adjacency=adjacency, edge_features=edge_features
        )
    ...
    def supervised_loss(self, batch: dict[str, Any]) -> torch.Tensor:
        ...
        output = self.forward(
            batch, decode_type="greedy", target_actions=target.actions
        )
        if output.log_probs is None:
            raise RuntimeError("Autoregressive decoder did not return log_probs")
        valid_steps = (
            (target.actions >= 0).sum(dim=1).clamp_min(1).to(output.log_probs.dtype)
        )
    ...
    def _sigmoid_supervised_loss(
        ...
        if target_mask is None:
            target_mask = _mask_from_actions(
                target.actions, logits.size(1), logits.device
            )
        if target_mask is None:
