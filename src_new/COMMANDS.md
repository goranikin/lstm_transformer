...
```bash
uv run python -m compileall -q src_new
```

Dry-run a small routing matrix:

```bash
uv run python -m src_new.experiments.matrix \
  --stage routing \
  --seeds 1234 \
  --modes supervised \
  --epochs 1 \
...
```bash
uv run python -m src_new.experiments.run \
  --problem tsp \
  --encoder attention \
  ...
  --epochs 100 \
  --batch-size 512 \
  --output-dir outputs/src_new/single/tsp_attention_attention_pointer_sl
```
...
uv run python -m src_new.experiments.run \
  ...
  --output-dir outputs/src_new/single/knapsack_attention_sigmoid_rl
```
...
```bash
uv run python -m src_new.experiments.matrix --stage all
```
...
uv run python -m src_new.experiments.matrix \
  ...
  --steps-per-epoch 100 \
  --batch-size 128 \
  --output-root outputs/src_new/pilot \
  --execute
```
...
uv run python -m src_new.experiments.matrix \
  ...
  --steps-per-epoch 100 \
  --batch-size 128 \
  --output-root outputs/src_new/pilot_mixed \
  --execute
```
...
uv run python -m src_new.experiments.matrix \
  ...
  --output-root outputs/src_new/custom_data
```
...
uv run python -m src_new.experiments.run \
  ...
  --steps-per-epoch 1 \
  --batch-size 4 \
  --output-dir /tmp/src_new_knapsack_debug \
  --no-progress \
  --no-checkpoints
```
