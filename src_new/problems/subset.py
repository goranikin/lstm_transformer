...
class OrienteeringProblem(Problem):
    ...
    def get_mask(self, state: ProblemState) -> torch.Tensor:
        ...
        feasible = path_length.view(-1, 1) + step + return_home <= budget.view(-1, 1) + 1e-12
    ...
    def context_features(self, state: ProblemState) -> torch.Tensor:
        ...
        available_ratio = ((~state.selected_mask[:, 1:]) & feasible[:, 1:]).float().mean(dim=1)
        return torch.stack([selected_ratio, available_ratio, remaining_budget, state.done.float()], dim=1)
    ...
    def repair_solution(
        ...
        for row in range(scores.size(0)):
            ...
            candidates = _candidate_order(scores[row], proposed_mask[row] if proposed_mask is not None else None, skip_zero=True)
...
class KnapsackProblem(Problem):
    ...
    def build_features(self, batch: dict[str, Any]):
        ...
        features = torch.stack([weights / capacity.clamp_min(1.0), values / value_scale], dim=-1)
    ...
    def make_state(self, batch: dict[str, Any]) -> ProblemState:
        ...
        return ProblemState(
            batch=batch,
            selected_mask=torch.zeros(batch_size, item_count, dtype=torch.bool, device=device),
            done=torch.zeros(batch_size, dtype=torch.bool, device=device),
    ...
    def step(self, state: ProblemState, action: torch.Tensor) -> ProblemState:
        ...
        if item_active.any():
            ...
            state.aux["remaining_capacity"][item_active] -= weights[rows[item_active], action[item_active]]
        self.append_action(state, action)
    ...
    def context_features(self, state: ProblemState) -> torch.Tensor:
        ...
        return torch.stack([selected_ratio, available_ratio, remaining_ratio, state.done.float()], dim=1)
    ...
    def check_feasible(self, batch: dict[str, Any], solution: dict[str, torch.Tensor]):
        weights = self.require_tensor(batch, "weights").float()
        capacity = self.require_tensor(batch, "capacity").float()
        return (solution["selected_mask"].float() * weights).sum(dim=1) <= capacity + 1e-12
    ...
    def repair_solution(
        ...
        for row in range(scores.size(0)):
            ...
            row_actions: list[int] = []
            for item in _candidate_order(scores[row], proposed_mask[row] if proposed_mask is not None else None):
            ...
            padded = torch.full((scores.size(1) + 1,), -1, dtype=torch.long, device=scores.device)
            padded[: len(row_actions)] = torch.tensor(row_actions, dtype=torch.long, device=scores.device)
...
def _candidate_order(
    ...
    *,
    ...
    return [int(index) for index in scores[start:].argsort(descending=True).add(start).tolist()]
