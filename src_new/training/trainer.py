...
from src_new.constants import TrainingMode
from src_new.model import NCOModel
from src_new.training.baselines import ExponentialRewardBaseline, RolloutRewardBaseline
from src_new.training.metrics import (
    EvaluationMetrics,
    aggregate_metrics,
    batch_metrics,
)
from src_new.utils import move_to_device, timer
...
class TrainingConfig:
    ...
    output_dir: str = "outputs/src_new"
...
class Trainer:
    ...
    def _train_rl_epoch(self, epoch: int) -> float:
        self.model.train()
        losses: list[float] = []
        if self.config.baseline == "rollout" and self.rollout_baseline.baseline_model is None:
    ...
    def evaluate(self, loader: DataLoader) -> EvaluationMetrics:
        ...
        for batch in loader:
            ...
            items.append(batch_metrics(self.model.problem, batch, output, elapsed["elapsed"]))
    ...
    def _write_history(self, result: TrainingResult) -> None:
        ...
        with open(os.path.join(self.config.output_dir, "history.json"), "w", encoding="utf-8") as handle:
